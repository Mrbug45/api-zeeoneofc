...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------KotzGanteng'+'KOTZ--API';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'KotzApi') return res.json(loghandler.invalidKey)
...
router.get('/tiktok', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'KotzApi') return res.json(loghandler.invalidKey)
...
router.get('/tiyurl', async (req, res, next) => {
    var apikeyInput = req.query.apikey,
        url = req.query.url
                ...
                body: `text_1=${text}&text_2=${text2}&login=OK`,
                    ...
                    if (!e) {
                        ...
                        $(".thumbnail").find("img").each(function() {
                            ...
                            fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=600&key=1c04eee640c8ae8cb7f9cd894e13b30a&image=${result}&name=${randomTextNumber}`))
                                .then(response => response.json())
                                .then(data => {
...
router.get('/ytv', async (req, res, next) => {
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
       ...
       fetch(encodeURI(`https://lolhuman.herokuapp.com/api/ytvideo?apikey=MiraCans&url=${url}`))
        .then(response => response.json())
        .then(data => {
